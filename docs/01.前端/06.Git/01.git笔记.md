...
> git是我们在工作中使用最频繁的工具之一，针对git的操作是非常多的，工作中我们可以使用git命令操作，也可以使用可视化工具
>
...
在提交代码时输入清晰的说明有利于版本管理，建议搭配如下关键字编写提交
...
- 第1步：创建`SSH Key`。在用户主（C:\Users\dell）目录下，看看有没有.ssh目录， 如果有，再看看这个目录下有没有`id_rsa`和`id_rsa.pub`这两个文件，如果已经有了，可直接跳到下一步。 如果没有，打开Shell（Windows下打开Git Bash），创建`SSH Key`：
...
# 示例
git remote add origin git@github.com:comelong.git
```
...
```bash
我们需要将开发分支的代码合并到master分支上
第一步:将开发分支mjl_dev分支代码提交到git远程端
第二步:git checkout master 切换到master分支上
第三步:git merge mjl_dev 即可将mjl_dev开发分支的代码合并到master分支上
第四步:如果有冲突，使用vscode编辑器解决冲突
第五步:解决冲突后，提交git push origin master 提交主分支代码到远端git
```
...
```bash
git branch -m 需要修改的分支名  新的分支名
git push origin :需要修改的分支名
